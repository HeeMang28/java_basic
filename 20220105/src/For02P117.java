...
public class For02P117 {
	...
	public static void main(String[] args) {
		...
		// 굉장히 많은 분야에서 응용되고 있으므로 반드시 잘 익혀둬야 한다.
		for(int i= 0; i < 3; i++) {// 2바퀴 도는 반복문 
...
// 11 : 주석이므로 무시
// 12 : j for문 시작(j = 0) j for문 비교(j < 2), (0 < 2) 참이므로 13번 진입
...
// 14 : j for문 닫는부분을 만나 12번(j 여는부분)으로 이동
// 12 : j++로 증가 (j = 1 => j = 2)
// 12 : j for문 비교(j < 2), (2 < 2) 거짓이므로 15번으로 탈출
// 15 : i for문 닫는부분을 만나 10번(i 여는부분)으로 이동
// 10 : i++로 1증가 (i = 0 => i = 1)
// 10 : i for문 비교 (j < 2), (1 < 2) 참이므로 11번 진입
// 11 : 주석이므로 무시
// 12 : j for문 시작(j = 0) j for문 비교(j < 2), (0 < 2) 참이므로 13번 진입
// 13 : sysout("Hello Java") 실행
// 14 : j for문 닫는부분을 만나 12번(j 여는부분)으로 이동
...
// 15 : i for문 닫는부분을 만나 10번(i 여는부분)으로 이동
// 10 : i++로 1증가 (i = 1 => i = 2)
// 10 : i for문 비교 (i < 2), (2 < 2) 거짓이므로 16으로 탈출
